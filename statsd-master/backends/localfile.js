  ...
  //if (graphiteHost) {
    try {
      ...
      //graphite.on('connect', function() {
        ...
        //Following changes made by JRK to support local file logging, need to figure how to add to config and retrieve into here
        //fs.appendFile("logs/stats.log", statString);
        fs.appendFile(logFile, statString);
        if (debug) {
          console.log(statString + " was appended to the file" );
        }
...
exports.init = function graphite_init(startup_time, config, events, logger) {
  ...
  //JRK added following for local config
  config.graphite_local = config.graphite_local || {};
  logFile = config.graphite_local.logFile;
